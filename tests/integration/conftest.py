...
def is_complytime_installed(install_dir: Path) -> bool:
    ...
    openscap_plugin_conf = (install_dir / '.config/complytime/plugins/c2p-openscap-manifest.json').resolve()
...
def run_before_and_after_tests():
    ...
    if not is_complytime_installed(complytime_home):
        ...
        result = subprocess.run(
            ['find', f'{complytime_cache_dir}/releases', '-name', 'complytime_linux_x86_64.tar.gz', '-exec', 'tar', '-xvf', '{}', ';', '-quit'],
            cwd=complytime_home,
        )
        if result.returncode != 0:
        ...
        shutil.move(complytime_home / 'openscap-plugin', complytime_home / '.config/complytime/plugins/openscap-plugin')
        openscap_plugin_sha256 = sha256sum(complytime_home / '.config/complytime/plugins/openscap-plugin')

        with open(int_test_data_dir / 'c2p-openscap-manifest.json') as c2p_openscap_manifest_file:
            c2p_openscap_manifest = json.load(c2p_openscap_manifest_file)
            c2p_openscap_manifest['sha256'] = openscap_plugin_sha256
            with open(complytime_home / '.config/complytime/plugins/c2p-openscap-manifest.json', 'w') as templated_file:
                json.dump(c2p_openscap_manifest, templated_file)

        # Create dummy base files
        shutil.copy(int_test_data_dir / 'sample-catalog.json', complytime_home / '.config/complytime/controls/sample-catalog.json')
        shutil.copy(int_test_data_dir / 'sample-profile.json', complytime_home / '.config/complytime/controls/sample-profile.json')
        shutil.copy(int_test_data_dir / 'sample-component-definition.json', complytime_home / '.config/complytime/bundles/sample-component-definition.json')
